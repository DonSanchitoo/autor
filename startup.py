...
def restaurer_interface_par_defaut():
    ...
    try:
        ...
        # 3. CODE MODIFIÉ : Rendre visibles SEULEMENT les docks Couches et Explorateur
        for dock in mw.findChildren(QDockWidget):
             dock_name = dock.objectName()
             
             # On masque par défaut, puis on affiche ceux qui sont nécessaires.
        ...
        iface.messageBar().pushInfo("Démarrage", "Interface restaurée (mode Maintenance/Normal).")
...
if is_key_pressed(VK_M) and is_key_pressed(VK_E):
    ...
    # --- BLOC ELSE : Démarrage normal (Personnalisation et contraintes) ---
    
    # ------------------------------------------------------------------
    # --- DÉFINITION DE LA FONCTION DE PERSONNALISATION ---
    # ------------------------------------------------------------------
    ...
    # --- Ajout d'un bouton pour ouvrir la table attributaire de la couche sélectionnée ---
    ...
    # Ajout du bouton à une barre d'outils existante (par exemple, la barre de sélection)
    toolbar = iface.mainWindow().findChild(QToolBar, 'mSelectionToolBar')
    if toolbar:
        toolbar.addAction(action_table)
    else:
        # Si la barre n'existe pas, on peut en créer une nouvelle
    ...
    # --- Splash screen discret (Votre code) ---
    splash = QWidget()
    splash.setWindowFlags(Qt.WindowStaysOnTopHint | Qt.FramelessWindowHint | Qt.Tool)
    splash.setAttribute(Qt.WA_TranslucentBackground)
    splash.setFixedSize(400, 80)
